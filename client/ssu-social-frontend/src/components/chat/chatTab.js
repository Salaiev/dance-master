...
const ChatTab = ({ chatRoom, chatRoomMessages, currentUser, chatUser }) => {
  ...
  const getChatHistoryStr = () => {
    ...
    lastTenMessages.forEach((message) => {
      ...
      const msg = String(message.text || "").replace("\n", " ").replace("\t", " ");
    ...
    <div className="h-full flex flex-col">
          ...
          <React.Fragment key={message._id}>
            ...
            >
              ...
              <div className="flex justify-end items-end w-3">
                {message.isRead && <div className="text-xs font-bold">{"✔"}</div>}
              </div>
            </div>
          </React.Fragment>
        ))}
      ...
      <div className="px-2 pt-2 flex items-center">
        ...
        <button
          ...
          title="Send"
        >
          <FontAwesomeIcon className="text-white-500 text-2xl" icon={sendIcon} />
        </button>
      </div>
    </div>
  );
